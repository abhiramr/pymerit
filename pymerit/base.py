...
class MeritMeta(abc.ABCMeta):
    ...
    def __init__(cls, name, bases, dct):
        if cls.__name__ not in ['MeritBase']:
            if not hasattr(cls, '_registry'):
                MeritBase._registry = []
            MeritBase.validate_cls(dct)        
            MeritBase._registry.append(cls)

        # Now initialize 
        super().__init__(name, bases, dct)

    def validate_schema(self, dct):
